...
public:
	...
	void AddPerson() {
		...
		switch (select) {
		case 1: Student newStudent(++_idMemory);
			_students.Add(newStudent,_userName);
			SaveToFile(newStudent.ToFile(), 's');
			//прогресс будет сохраняться в другом файле прогрессов (id|subject|5,4,5,3,5,3,4)
			newStudent.ProgressToFile();
			break;
		case 2: Employee newEmployee(++_idMemory);
			_employees.Add(newEmployee,_userName);
			SaveToFile(newEmployee.ToFile(), 'e');
			break;
		case 3: Teacher newTeacher(++_idMemory);
	...
	//запись производится в 3 файла .csv для баз данных, 1 файл прогресса .csv, 1 текстовый лог-файл, 1 бинарный файл для запоминания id
};
